...
# Ханойская башня
  Это классическая математическая головоломка, которая используется для иллюстрации рекурсивных алгоритмов.\
  Задача была придумана французским математиком Эдуардом Люка в 1883 году.

  ## Описание задачи
  Есть три стержня (обычно обозначаются как A, B и C) и n дисков разного размера, которые могут надеваться на эти стержни.\
  Изначально все диски находятся на стержне A, упорядоченные по размеру: самый маленький сверху, а самый большой снизу.

  **Цель**: Переместить все диски с стержня A на стержень C, соблюдая следующие правила:

   - За один ход можно перемещать только один диск.
   - Нельзя класть больший диск на меньший.
   - Можно использовать стержень B как вспомогательный.

  **Рекурсивное решение**  
  Задача решается с помощью рекурсии. Основная идея:
   - Переместить n−1 верхних дисков с стержня A на стержень B, используя C как вспомогательный.
   - Переместить оставшийся самый большой диск с стержня A на стержень C.
   - Переместить n−1 дисков с стержня B на стержень C, используя A как вспомогательный.

  ## Легенда

  Легенда о Ханойской башне — это красивая и поучительная история, которая придаёт задаче особый шарм и делает её ещё более интересной. Вот как она звучит:\
  Давным-давно, в одном из храмов Ханоя (Вьетнам), монахи занимались перемещением 64 золотых дисков между тремя алмазными стержнями.\
  Диски были разного размера и изначально находились на одном стержне, упорядоченные по размеру: самый маленький сверху, а самый большой снизу.\
  Монахи верили, что когда они закончат перемещение всех дисков на другой стержень, наступит конец света.\
  Согласно легенде, они выполняли эту задачу, следуя строгим правилам:
  - За один ход можно перемещать только один диск.
  - Нельзя класть больший диск на меньший.
  - Можно использовать третий стержень как вспомогательный.

  **Математический смысл**\
  Количество шагов, необходимых для перемещения n дисков, вычисляется по формуле:
  
  Количество шагов = 2^n − 1

  Для n n=64 количество шагов равно:
   2 ^ 64 − 1 ≈ 18.4 * 10^18

  Если монахи будут делать по одному ходу в секунду, им потребуется примерно 585 миллиардов лет, чтобы завершить задачу.\
  Это намного больше, чем возраст Вселенной (около 13.8 миллиардов лет).

  Легенда о Ханойской башне — это не только красивая история, но и отличный способ понять,\
  как рекурсия и математика помогают решать сложные задачи.\
  Она напоминает нам, что даже самые грандиозные цели можно достичь, если разбить их на маленькие шаги.
...
# Описание файлов
 ...
 - dynamicArray.cpp - алгоритмы работы с двумерным динамическим массивом. Реализация с помощью шаблонов
 - hanoi.cpp - решение задачи "Ханойские башни" без использования рекурсии
 - hanoi_rec.cpp - решение задачи "Ханойские башни" с использованием рекурсии
 - lesson_today.cpp - матерал разобранный на прошлом занятии и на текущем. По динамическим массивам
 - recurs.cpp - задачи на тему рекурсия
